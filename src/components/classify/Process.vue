...
export default {
  ...
  computed: {
    changeTime () {
      const avg = this.restTime / this.allTime
  ...
  methods: {
    ...
    move (e) {
      ...
      const changeX = this.lef + nowX - this.currentX
  ...
  mounted () {
    ...
    if (this.isAuto && this.allTime && this.restTime) {
      const timeB = document.querySelector('.process_time')
      if (timeB) this.maxWidth -= timeB.offsetWidth
    }
  }
}
</script>
